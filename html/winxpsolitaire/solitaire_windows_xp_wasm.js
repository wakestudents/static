...
function allocate(slab, allocator) { var ret; if (allocator == ALLOC_STACK) { ret = stackAlloc(slab.length) } else { ret = _malloc(slab.length) } if (slab.subarray || slab.slice) { HEAPU8.set(slab, ret) } else { HEAPU8.set(new Uint8Array(slab), ret) } return ret }
...
if (Module["wasmMemory"]) { wasmMemory = Module["wasmMemory"] } else { wasmMemory = new WebAssembly.Memory({ "initial": INITIAL_MEMORY / 65536, "maximum": 2147483648 / 65536 }) }
...
function getBinaryPromise() { if (!wasmBinary && (ENVIRONMENT_IS_WEB || ENVIRONMENT_IS_WORKER) && typeof fetch === "function" && !isFileURI(wasmBinaryFile)) { return fetch(wasmBinaryFile, { credentials: "same-origin" }).then(function(response) { if (!response["ok"]) { throw "failed to load wasm binary file at '" + wasmBinaryFile + "'" } return response["arrayBuffer"]() }).catch(function() { return getBinary(wasmBinaryFile) }) } return Promise.resolve().then(function() { return getBinary(wasmBinaryFile) }) }
...
var ASM_CONSTS = {
    6951: function($0, $1, $2) {
        Module.__defosjs_mouseenter_listener = function() { _defos_emit_event_from_js($0) };
        Module.__defosjs_mouseleave_listener = function() { _defos_emit_event_from_js($1) };
        Module.__defosjs_click_listener = function() { _defos_emit_event_from_js($2) };
    ...
    9679: function() { Module.toggleFullscreen() },
    9706: function() {
        Module.canvas.width = window.innerWidth;
        Module.canvas.height = window.innerHeight
    },
    9794: function() { return GLFW.isFullscreen },
    9880: function($0) { document.title = UTF8ToString($0) },
    9916: function($0) {
    ...
    10750: function() { return Module.canvas.width },
    10782: function() { return Module.canvas.height },
    10873: function($0) { Module.canvas.style.cursor = UTF8ToString($0) },
    10922: function() { Module.canvas.style.cursor = "none" },
    10959: function() { return Module.__defosjs_mouse_x },
    10997: function() { return Module.__defosjs_mouse_y },
    118102: function($0) {
        var jsResult;
        var isSuccess = 1;
        try { jsResult = eval(UTF8ToString($0)) } catch (err) {
            isSuccess = 0;
            jsResult = err
        }
...
function ___gmtime_r(a0, a1) { return _gmtime_r(a0, a1) }
...
function ___sys_dup2(oldfd, suggestFD) { try { var old = SYSCALLS.getStreamFromFD(oldfd); if (old.fd === suggestFD) return suggestFD; return SYSCALLS.doDup(old.path, old.flags, suggestFD) } catch (e) { if (typeof FS === "undefined" || !(e instanceof FS.ErrnoError)) abort(e); return -e.errno } }

function ___sys_dup3(fd, suggestFD, flags) { try { var old = SYSCALLS.getStreamFromFD(fd); if (old.fd === suggestFD) return -28; return SYSCALLS.doDup(old.path, old.flags, suggestFD) } catch (e) { if (typeof FS === "undefined" || !(e instanceof FS.ErrnoError)) abort(e); return -e.errno } }
...
function ___sys_lstat64(path, buf) { try { path = SYSCALLS.getStr(path); return SYSCALLS.doStat(FS.lstat, path, buf) } catch (e) { if (typeof FS === "undefined" || !(e instanceof FS.ErrnoError)) abort(e); return -e.errno } }

function ___sys_mkdir(path, mode) { try { path = SYSCALLS.getStr(path); return SYSCALLS.doMkdir(path, mode) } catch (e) { if (typeof FS === "undefined" || !(e instanceof FS.ErrnoError)) abort(e); return -e.errno } }

function ___sys_open(path, flags, varargs) { SYSCALLS.varargs = varargs; try { var pathname = SYSCALLS.getStr(path); var mode = varargs ? SYSCALLS.get() : 0; var stream = FS.open(pathname, flags, mode); return stream.fd } catch (e) { if (typeof FS === "undefined" || !(e instanceof FS.ErrnoError)) abort(e); return -e.errno } }
    ...
    websocket_sock_ops: {
        createPeer: function(sock, addr, port) {
            ...
            if (ws) {
                ...
                try {
                    ...
                    if (ENVIRONMENT_IS_NODE) { WebSocketConstructor = require("ws") } else { WebSocketConstructor = WebSocket }
        ...
        bind: function(sock, addr, port) {
            ...
            if (sock.type === 2) {
                ...
                try { sock.sock_ops.listen(sock, 0) } catch (e) { if (!(e instanceof FS.ErrnoError)) throw e; if (e.errno !== ERRNO_CODES.EOPNOTSUPP) throw e }
            }
        },
        ...
        sendmsg: function(sock, buffer, offset, length, addr, port) {
            ...
            try { dest.socket.send(data); return length } catch (e) { throw new FS.ErrnoError(ERRNO_CODES.EINVAL) }
        },
        recvmsg: function(sock, length) {
            if (sock.type === 1 && sock.server) { throw new FS.ErrnoError(ERRNO_CODES.ENOTCONN) }
            var queued = sock.recv_queue.shift();
            if (!queued) { if (sock.type === 1) { var dest = SOCKFS.websocket_sock_ops.getPeer(sock, sock.daddr, sock.dport); if (!dest) { throw new FS.ErrnoError(ERRNO_CODES.ENOTCONN) } else if (dest.socket.readyState === dest.socket.CLOSING || dest.socket.readyState === dest.socket.CLOSED) { return null } else { throw new FS.ErrnoError(ERRNO_CODES.EAGAIN) } } else { throw new FS.ErrnoError(ERRNO_CODES.EAGAIN) } }
...
function ___sys_socketcall(call, socketvararg) {
    try {
        ...
        var getSocketFromFD = function() { var socket = SOCKFS.getSocket(SYSCALLS.get()); if (!socket) throw new FS.ErrnoError(8); return socket };
            ...
            case 5:
                {
                    ...
                    if (addr) { var errno = __write_sockaddr(addr, newsock.family, DNS.lookup_name(newsock.daddr), newsock.dport, addrlen) }
            ...
            case 12:
                {
                    ...
                    if (!msg) return 0;
                    if (addr) { var errno = __write_sockaddr(addr, sock.family, DNS.lookup_name(msg.addr), msg.port, addrlen) }
            ...
            case 16:
                {
                    ...
                    var offset = 0;
                    for (var i = 0; i < num; i++) { var iovbase = HEAP32[iov + (8 * i + 0) >> 2]; var iovlen = HEAP32[iov + (8 * i + 4) >> 2]; for (var j = 0; j < iovlen; j++) { view[offset++] = HEAP8[iovbase + j >> 0] } }
            ...
            case 17:
                {
                    ...
                    if (!msg) return 0;
                    var name = HEAP32[message >> 2];
                    if (name) { var errno = __write_sockaddr(name, sock.family, DNS.lookup_name(msg.addr), msg.port) }
                    var bytesRead = 0;
...
var DefoldSoundDevice = { TryResumeAudio: function() { var audioCtx = window._dmJSDeviceShared.audioCtx; if (audioCtx !== undefined && audioCtx.state != "running") { audioCtx.resume() } } };

function _dmDeviceJSFreeBufferSlots(id) { return window._dmJSDeviceShared.devices[id]._freeBufferSlots() }
...
function _dmDeviceJSOpen(bufferCount) {
    ...
    if (window.AudioContext || window.webkitAudioContext) {
        if (shared.audioCtx === undefined) { shared.audioCtx = new(window.AudioContext || window.webkitAudioContext) }
        device = {
            ...
            _freeBufferSlots: function() { if (this.bufferDuration == 0) return 1; var ahead = this.bufferedTo - shared.audioCtx.currentTime; var inqueue = Math.ceil(ahead / this.bufferDuration); if (inqueue < 0) { inqueue = 0 } var left = bufferCount - inqueue; if (left < 0) { return 0 } return left }
        }
    }
    if (device != null) { shared.devices[id] = device; return id }
    return -1
}

function _dmDeviceJSQueue(id, samples, sample_count) { window._dmJSDeviceShared.devices[id]._queue(samples, sample_count) }

function _dmGetDeviceSampleRate(id) { return window._dmJSDeviceShared.devices[id].sampleRate }
...
function _dmScriptHttpRequestAsync(method, url, headers, arg, onload, onerror, send_data, send_data_length, timeout) {
    ...
    function listener() {
        ...
        if (xhr.status != 0) {
            ...
            _free(b)
        } else { wasmTable.get(onerror)(arg, xhr.status) }
    }
    xhr.onload = listener;
    ...
    xhr.responseType = "arraybuffer";
    if (timeout > 0) { xhr.timeout = timeout / 1e3 }
    ...
    if (send_data_length > 0) { xhr.send(HEAPU8.subarray(send_data, send_data + send_data_length)) } else { xhr.send() }
}
var DMSYS = {
    _folder: "/data",
    _cstr: null,
...
function _dmSysOpenURL(url, target) { var jsurl = UTF8ToString(url); var jstarget = UTF8ToString(target); if (jstarget == 0) { jstarget = "_self" } if (window.open(jsurl, jstarget) == null) { window.location = jsurl } return true }
...
function _emscripten_asm_const_double(a0, a1, a2) { return _emscripten_asm_const_int(a0, a1, a2) }
...
var Browser = {
    ...
    createContext: function(canvas, useWebGL, setInModule, webGLContextAttributes) {
        ...
        var ctx;
        var contextHandle;
        if (useWebGL) { var contextAttributes = { antialias: false, alpha: false, majorVersion: 1 }; if (webGLContextAttributes) { for (var attribute in webGLContextAttributes) { contextAttributes[attribute] = webGLContextAttributes[attribute] } } if (typeof GL !== "undefined") { contextHandle = GL.createContext(canvas, contextAttributes); if (contextHandle) { ctx = GL.getContext(contextHandle).GLctx } } } else { ctx = canvas.getContext("2d") }
        if (!ctx) return null;
        if (setInModule) {
    ...
    safeSetTimeout: function(func, timeout) { noExitRuntime = true; return setTimeout(function() { if (ABORT) return; if (Browser.allowAsyncCallbacks) { func() } else { Browser.queuedAsyncCallbacks.push(func) } }, timeout) },
    safeSetInterval: function(func, timeout) { noExitRuntime = true; return setInterval(function() { if (ABORT) return; if (Browser.allowAsyncCallbacks) { func() } }, timeout) },
...
var GL = {
    ...
    initExtensions: function(context) {
        ...
        exts.forEach(function(ext) { if (ext.indexOf("lose_context") < 0 && ext.indexOf("debug") < 0) { GLctx.getExtension(ext) } })
    },
...
function _emscripten_glGenQueriesEXT(n, ids) {
    for (var i = 0; i < n; i++) {
        ...
        if (!query) { GL.recordError(1282); while (i < n) HEAP32[ids + i++ * 4 >> 2] = 0; return }
...
function _emscripten_glGetString(name_) {
    ...
    switch (name_) {
        ...
        case 37446:
            ...
            break;
        case 7938:
            var glVersion = GLctx.getParameter(7938); { glVersion = "OpenGL ES 2.0 (" + glVersion + ")" }
...
function __colorChannelsInGlTextureFormat(format) { var colorChannels = { 5: 3, 6: 4, 8: 2, 29502: 3, 29504: 4 }; return colorChannels[format - 6402] || 1 }

function heapObjectForWebGLType(type) { type -= 5120; if (type == 1) return HEAPU8; if (type == 4) return HEAP32; if (type == 6) return HEAPF32; if (type == 5 || type == 28922) return HEAPU32; return HEAPU16 }
...
function _emscripten_longjmp(a0, a1) { return _longjmp(a0, a1) }

function _emscripten_memcpy_big(dest, src, num) { HEAPU8.copyWithin(dest, src, src + num) }

function _emscripten_pause_main_loop() { Browser.mainLoop.pause() }
...
function _getaddrinfo(node, service, hint, out) {
    ...
    } else { addr = __inet_pton6_raw(node); if (addr !== null) { if (family === 0 || family === 10) { family = 10 } else { return -2 } } }
    if (addr != null) {
...
function _gethostbyaddr(addr, addrlen, type) {
    if (type !== 2) { setErrNo(5); return null }
    addr = HEAP32[addr >> 2];
    var host = __inet_ntop4_raw(addr);
    var lookup = DNS.lookup_addr(host);
    if (lookup) { host = lookup }
...
function _glBufferSubData(target, offset, size, data) { GLctx.bufferSubData(target, offset, HEAPU8.subarray(data, data + size)) }
...
function _glCompressedTexSubImage2D(target, level, xoffset, yoffset, width, height, format, imageSize, data) { GLctx["compressedTexSubImage2D"](target, level, xoffset, yoffset, width, height, format, data ? HEAPU8.subarray(data, data + imageSize) : null) }
...
function _glGetActiveUniform(program, index, bufSize, length, size, type, name) { __glGetActiveAttribOrUniform("getActiveUniform", program, index, bufSize, length, size, type, name) }
...
function _glGetProgramInfoLog(program, maxLength, length, infoLog) { var log = GLctx.getProgramInfoLog(GL.programs[program]); if (log === null) log = "(unknown error)"; var numBytesWrittenExclNull = maxLength > 0 && infoLog ? stringToUTF8(log, infoLog, maxLength) : 0; if (length) HEAP32[length >> 2] = numBytesWrittenExclNull }
...
var GLFW = {
    ...
    DOMtoGLFWButton: function(button) { if (button == 1) { button = 2 } else if (button == 2) { button = 1 } return button },
    ...
    addEventListener: function(type, listener, useCapture) { if (typeof window !== "undefined") { window.addEventListener(type, listener, useCapture) } },
    removeEventListener: function(type, listener, useCapture) { if (typeof window !== "undefined") { window.removeEventListener(type, listener, useCapture) } },
    addEventListenerCanvas: function(type, listener, useCapture) { if (typeof Module["canvas"] !== "undefined") { Module["canvas"].addEventListener(type, listener, useCapture) } },
    removeEventListenerCanvas: function(type, listener, useCapture) { if (typeof Module["canvas"] !== "undefined") { Module["canvas"].removeEventListener(type, listener, useCapture) } },
    isCanvasActive: function(event) { var res = typeof document.activeElement == "undefined" || document.activeElement == Module["canvas"]; if (!res) { res = event.target == Module["canvas"] } if (event.target.focus) event.target.focus(); return res },
    onKeyPress: function(event) { if (!GLFW.isCanvasActive(event)) { return } if (event.charCode) { var char = GLFW.getUnicodeChar(event.charCode); if (char !== null && GLFW.charFunc) { wasmTable.get(GLFW.charFunc)(event.charCode, 1) } } },
    onKeyChanged: function(event, status) { if (!GLFW.isCanvasActive(event)) { return } var key = GLFW.DOMToGLFWKeyCode(event.keyCode); if (key) { GLFW.keys[key] = status; if (GLFW.keyFunc) { wasmTable.get(GLFW.keyFunc)(key, status) } } },
    onKeydown: function(event) {
        ...
        } else if (event.keyCode === 8 || event.keyCode === 9 || event.keyCode === 13) { event.preventDefault() }
    },
    ...
    onMouseButtonChanged: function(event, status) {
        if (!GLFW.isCanvasActive(event)) { return }
        if (GLFW.mouseButtonFunc == null) { return }
        Browser.calculateMouseEvent(event);
        if (event.target != Module["canvas"]) { return }
        if (status == 1) { try { event.target.setCapture() } catch (e) {} }
        event.preventDefault();
        var eventButton = GLFW.DOMtoGLFWButton(event["button"]);
        wasmTable.get(GLFW.mouseButtonFunc)(eventButton, status)
    },
    fillTouch: function(id, x, y, phase) { if (GLFW.touchFunc) { wasmTable.get(GLFW.touchFunc)(id, x, y, phase) } },
    touchWasFinished: function(event, phase) {
        ...
        if (event.touches.length == 0) { GLFW.buttons &= ~(1 << 0) }
        if (typeof DefoldSoundDevice != "undefined" && DefoldSoundDevice != null) { DefoldSoundDevice.TryResumeAudio() }
        event.preventDefault()
    },
    onTouchEnd: function(event) { GLFW.touchWasFinished(event, GLFW.GLFW_PHASE_ENDED) },
    onTouchCancel: function(event) { GLFW.touchWasFinished(event, GLFW.GLFW_PHASE_CANCELLED) },
    convertCoordinatesFromMonitorToWebGLPixels: function(x, y) { var rect = Module["canvas"].getBoundingClientRect(); var canvasWidth = rect.right - rect.left; var canvasHeight = rect.bottom - rect.top; var canvasX = x - rect.left; var canvasY = y - rect.top; var canvasXNormalized = canvasX / canvasWidth; var canvasYNormalized = canvasY / canvasHeight; var finalX = Module["canvas"].width * canvasXNormalized; var finalY = Module["canvas"].height * canvasYNormalized; return [finalX, finalY] },
    ...
    onMouseWheel: function(event) {
        ...
        if (event.target == Module["canvas"]) {
            if (GLFW.mouseWheelFunc) { wasmTable.get(GLFW.mouseWheelFunc)(GLFW.wheelPos) }
            event.preventDefault()
        }
    },
    onFocusChanged: function(focus) { if (GLFW.focusFunc) { wasmTable.get(GLFW.focusFunc)(focus) } },
    ...
    onJoystickConnected: function(event) { GLFW.refreshJoysticks() },
    onJoystickDisconnected: function(event) { GLFW.refreshJoysticks(true) },
    disconnectJoystick: function(joy) {
        _free(GLFW.joys[joy].id);
        delete GLFW.joys[joy];
        if (GLFW.gamepadFunc) { wasmTable.get(GLFW.gamepadFunc)(joy, 0) }
    },
    joys: {},
            ...
            for (var joy = 0; joy < GLFW.lastGamepadState.length; ++joy) {
                ...
                if (gamepad) {
                    if (!GLFW.joys[joy] || GLFW.joys[joy].id_string != gamepad.id) {
                        ...
                        if (GLFW.gamepadFunc) { wasmTable.get(GLFW.gamepadFunc)(joy, 1) }
                    }
                    GLFW.joys[joy].buttons = gamepad.buttons;
                    GLFW.joys[joy].axes = gamepad.axes
                } else { if (GLFW.joys[joy]) { GLFW.disconnectJoystick(joy) } }
            }
        }
    }
};
...
function _glfwCloseWindow() {
    if (GLFW.closeFunc) { wasmTable.get(GLFW.closeFunc)() }
    Module.ctx = Browser.destroyContext(Module["canvas"], true, true)
}

function _glfwGetAcceleration(x, y, z) { return 0 }

function _glfwGetDefaultFramebuffer() { return 0 }

function _glfwGetJoystickButtons(joy, buttons, numbuttons) { GLFW.refreshJoysticks(); var state = GLFW.joys[joy]; if (!state || !state.buttons) { for (var i = 0; i < numbuttons; i++) { setValue(buttons + i, 0, "i8") } return } for (var i = 0; i < Math.min(numbuttons, state.buttonsCount); i++) { setValue(buttons + i, state.buttons[i].pressed, "i8") } }

function _glfwGetJoystickDeviceId(joy, device_id) { if (GLFW.joys[joy]) { setValue(device_id, GLFW.joys[joy].id, "*"); return 1 } else { return 0 } }
...
function _glfwGetJoystickPos(joy, pos, numaxes) { GLFW.refreshJoysticks(); var state = GLFW.joys[joy]; if (!state || !state.axes) { for (var i = 0; i < numaxes; i++) { setValue(pos + i * 4, 0, "float") } return } for (var i = 0; i < numaxes; i++) { setValue(pos + i * 4, state.axes[i], "float") } }

function _glfwGetKey(key) { return GLFW.keys[key] }

function _glfwGetMouseButton(button) { return (GLFW.buttons & 1 << GLFW.DOMtoGLFWButton(button)) > 0 }
...
function _glfwGetMouseWheel() { return GLFW.wheelPos }

function _glfwGetProcAddress(procname) { return _getProcAddress(procname) }

function _glfwGetWindowParam(param) { return GLFW.params[param] }

function _glfwGetWindowRefreshRate() { return 0 }
...
function _glfwInitJS() {
    ...
    __ATEXIT__.push({
        func: function() {
            ...
            var canvas = Module["canvas"];
            if (typeof canvas !== "undefined") { Module["canvas"].width = Module["canvas"].height = 1 }
        }
    });
...
function _glfwOpenWindow(width, height, redbits, greenbits, bluebits, alphabits, depthbits, stencilbits, mode) {
    ...
    if (width > 0 && height == 0) { height = 3 * width / 4 }
    ...
    } else { throw "Invalid glfwOpenWindow mode." }
...
function _glfwOpenWindowHint(target, hint) { GLFW.params[target] = hint; if (target == 131097) { if (hint != 0) { GLFW.dpi = window.devicePixelRatio || 1 } } }

function _glfwPollEvents() {}

function _glfwResetKeyboard() {}

function _glfwSetCharCallback(cbfun) { GLFW.charFunc = cbfun; return 1 }
...
function _glfwSetMarkedTextCallback(cbfun) { GLFW.markedTextFunc = cbfun; return 1 }

function _glfwSetTouchCallback(cbfun) { GLFW.touchFunc = cbfun; return 1 }

function _glfwSetWindowCloseCallback(cbfun) { GLFW.closeFunc = cbfun }

function _glfwSetWindowFocusCallback(cbfun) { GLFW.focusFunc = cbfun }

function _glfwSetWindowIconifyCallback(cbfun) { GLFW.iconifyFunc = cbfun }

function _glfwSetWindowSize(width, height) { Browser.setCanvasSize(width, height); if (GLFW.resizeFunc) { wasmTable.get(GLFW.resizeFunc)(width, height) } }

function _glfwSetWindowSizeCallback(cbfun) { GLFW.resizeFunc = cbfun }

function _glfwShowKeyboard(show_keyboard) { Module["canvas"].contentEditable = show_keyboard ? true : false; if (show_keyboard) { Module["canvas"].focus() } }
...
var listener = { onRemoteConfigsUpdated: function() { _js_remote_configs_callback() } };

function _js_addBusinessEvent(currency, amount, itemType, itemId, cartType, fields) { gameanalytics.GameAnalytics.addBusinessEvent(UTF8ToString(currency), amount, UTF8ToString(itemType), UTF8ToString(itemId), UTF8ToString(cartType)) }

function _js_addDesignEvent(eventId, fields) { gameanalytics.GameAnalytics.addDesignEvent(UTF8ToString(eventId)) }

function _js_addDesignEventWithValue(eventId, value, fields) { gameanalytics.GameAnalytics.addDesignEvent(UTF8ToString(eventId), value) }

function _js_addErrorEvent(severity, message, fields) { gameanalytics.GameAnalytics.addErrorEvent(severity, UTF8ToString(message)) }

function _js_addProgressionEvent(progressionStatus, progression01, progression02, progression03, fields) { gameanalytics.GameAnalytics.addProgressionEvent(progressionStatus, UTF8ToString(progression01), UTF8ToString(progression02), UTF8ToString(progression03)) }

function _js_addProgressionEventWithScore(progressionStatus, progression01, progression02, progression03, score, fields) { gameanalytics.GameAnalytics.addProgressionEvent(progressionStatus, UTF8ToString(progression01), UTF8ToString(progression02), UTF8ToString(progression03), score) }

function _js_addResourceEvent(flowType, currency, amount, itemType, itemId, fields) { gameanalytics.GameAnalytics.addResourceEvent(flowType, UTF8ToString(currency), amount, UTF8ToString(itemType), UTF8ToString(itemId)) }

function _js_configureAvailableCustomDimensions01(list) { gameanalytics.GameAnalytics.configureAvailableCustomDimensions01(JSON.parse(UTF8ToString(list))) }

function _js_configureAvailableCustomDimensions02(list) { gameanalytics.GameAnalytics.configureAvailableCustomDimensions02(JSON.parse(UTF8ToString(list))) }

function _js_configureAvailableCustomDimensions03(list) { gameanalytics.GameAnalytics.configureAvailableCustomDimensions03(JSON.parse(UTF8ToString(list))) }

function _js_configureAvailableResourceCurrencies(list) { gameanalytics.GameAnalytics.configureAvailableResourceCurrencies(JSON.parse(UTF8ToString(list))) }

function _js_configureAvailableResourceItemTypes(list) { gameanalytics.GameAnalytics.configureAvailableResourceItemTypes(JSON.parse(UTF8ToString(list))) }

function _js_configureBuild(build) { gameanalytics.GameAnalytics.configureBuild(UTF8ToString(build)) }

function _js_configureGameEngineVersion(version) { gameanalytics.GameAnalytics.configureGameEngineVersion(UTF8ToString(version)) }

function _js_configureSdkGameEngineVersion(version) { gameanalytics.GameAnalytics.configureSdkGameEngineVersion(UTF8ToString(version)) }

function _js_configureUserId(userId) { gameanalytics.GameAnalytics.configureUserId(UTF8ToString(userId)) }

function _js_endSession() { gameanalytics.GameAnalytics.endSession() }
...
function _js_isRemoteConfigsReady() { return gameanalytics.GameAnalytics.isRemoteConfigsReady() }

function _js_setCustomDimension01(customDimension) { gameanalytics.GameAnalytics.setCustomDimension01(UTF8ToString(customDimension)) }

function _js_setCustomDimension02(customDimension) { gameanalytics.GameAnalytics.setCustomDimension02(UTF8ToString(customDimension)) }

function _js_setCustomDimension03(customDimension) { gameanalytics.GameAnalytics.setCustomDimension03(UTF8ToString(customDimension)) }

function _js_setEnabledInfoLog(enabled) { gameanalytics.GameAnalytics.setEnabledInfoLog(enabled) }

function _js_setEnabledVerboseLog(enabled) { gameanalytics.GameAnalytics.setEnabledVerboseLog(enabled) }

function _js_setEventSubmission(enabled) { gameanalytics.GameAnalytics.setEnabledEventSubmission(enabled) }

function _js_setManualSessionHandling(enabled) { gameanalytics.GameAnalytics.setEnabledManualSessionHandling(enabled) }

function _js_startSession() { gameanalytics.GameAnalytics.startSession() }
...
function _mktime(tmPtr) {
    ...
    if (dst < 0) { HEAP32[tmPtr + 32 >> 2] = Number(summerOffset != winterOffset && dstOffset == guessedOffset) } else if (dst > 0 != (dstOffset == guessedOffset)) {
...
function _pthread_join() { return 28 }
...
function _system(command) {
    if (ENVIRONMENT_IS_NODE) {
        ...
        var _W_EXITCODE = function(ret, sig) { return ret << 8 | sig };
        if (ret.status === null) {
...
var asmLibraryArg = { "b": ___assert_fail, "Yf": ___clock_gettime, "Xf": ___gmtime_r, "Wf": ___localtime_r, "Uf": ___sys__newselect, "Lf": ___sys_dup2, "Qf": ___sys_dup3, "i": ___sys_fcntl64, "Mf": ___sys_ioctl, "Rf": ___sys_lstat64, "Of": ___sys_mkdir, "aa": ___sys_open, "Tf": ___sys_poll, "Pf": ___sys_rename, "Nf": ___sys_rmdir, "j": ___sys_socketcall, "Sf": ___sys_stat64, "Vf": ___sys_uname, "ba": ___sys_unlink, "_": _abort, "If": _clock, "Hf": defos_set_cursor_locked_, "Gf": _difftime, "Ff": _dmDeviceJSFreeBufferSlots, "Ef": _dmDeviceJSOpen, "Df": _dmDeviceJSQueue, "Cf": _dmGetDeviceSampleRate, "Bf": _dmScriptHttpRequestAsync, "Af": _dmSysGetApplicationPath, "zf": _dmSysGetUserAgent, "yf": _dmSysGetUserPersistentDataRoot, "xf": _dmSysGetUserPreferredLanguage, "wf": _dmSysOpenURL, "Ca": _emscripten_asm_const_double, "f": _emscripten_asm_const_int, "vf": _emscripten_cancel_main_loop, "O": _emscripten_get_now, "uf": _emscripten_glActiveTexture, "tf": _emscripten_glAttachShader, "sf": _emscripten_glBeginQueryEXT, "rf": _emscripten_glBindAttribLocation, "qf": _emscripten_glBindBuffer, "pf": _emscripten_glBindFramebuffer, "of": _emscripten_glBindRenderbuffer, "nf": _emscripten_glBindTexture, "mf": _emscripten_glBindVertexArrayOES, "lf": _emscripten_glBlendColor, "kf": _emscripten_glBlendEquation, "jf": _emscripten_glBlendEquationSeparate, "hf": _emscripten_glBlendFunc, "gf": _emscripten_glBlendFuncSeparate, "ff": _emscripten_glBufferData, "ef": _emscripten_glBufferSubData, "df": _emscripten_glCheckFramebufferStatus, "cf": _emscripten_glClear, "bf": _emscripten_glClearColor, "af": _emscripten_glClearDepthf, "$e": _emscripten_glClearStencil, "_e": _emscripten_glColorMask, "Ze": _emscripten_glCompileShader, "Ye": _emscripten_glCompressedTexImage2D, "Xe": _emscripten_glCompressedTexSubImage2D, "We": _emscripten_glCopyTexImage2D, "Ve": _emscripten_glCopyTexSubImage2D, "Ue": _emscripten_glCreateProgram, "Te": _emscripten_glCreateShader, "Se": _emscripten_glCullFace, "Re": _emscripten_glDeleteBuffers, "Qe": _emscripten_glDeleteFramebuffers, "Pe": _emscripten_glDeleteProgram, "Oe": _emscripten_glDeleteQueriesEXT, "Ne": _emscripten_glDeleteRenderbuffers, "Me": _emscripten_glDeleteShader, "Le": _emscripten_glDeleteTextures, "Ke": _emscripten_glDeleteVertexArraysOES, "Je": _emscripten_glDepthFunc, "Ie": _emscripten_glDepthMask, "He": _emscripten_glDepthRangef, "Ge": _emscripten_glDetachShader, "Fe": _emscripten_glDisable, "Ee": _emscripten_glDisableVertexAttribArray, "De": _emscripten_glDrawArrays, "Ce": _emscripten_glDrawArraysInstancedANGLE, "Be": _emscripten_glDrawBuffersWEBGL, "Ae": _emscripten_glDrawElements, "ze": _emscripten_glDrawElementsInstancedANGLE, "ye": _emscripten_glEnable, "xe": _emscripten_glEnableVertexAttribArray, "we": _emscripten_glEndQueryEXT, "ve": _emscripten_glFinish, "ue": _emscripten_glFlush, "te": _emscripten_glFramebufferRenderbuffer, "se": _emscripten_glFramebufferTexture2D, "re": _emscripten_glFrontFace, "qe": _emscripten_glGenBuffers, "pe": _emscripten_glGenFramebuffers, "oe": _emscripten_glGenQueriesEXT, "ne": _emscripten_glGenRenderbuffers, "me": _emscripten_glGenTextures, "le": _emscripten_glGenVertexArraysOES, "ke": _emscripten_glGenerateMipmap, "je": _emscripten_glGetActiveAttrib, "ie": _emscripten_glGetActiveUniform, "he": _emscripten_glGetAttachedShaders, "ge": _emscripten_glGetAttribLocation, "fe": _emscripten_glGetBooleanv, "ee": _emscripten_glGetBufferParameteriv, "de": _emscripten_glGetError, "ce": _emscripten_glGetFloatv, "be": _emscripten_glGetFramebufferAttachmentParameteriv, "ae": _emscripten_glGetIntegerv, "$d": _emscripten_glGetProgramInfoLog, "_d": _emscripten_glGetProgramiv, "Zd": _emscripten_glGetQueryObjecti64vEXT, "Yd": _emscripten_glGetQueryObjectivEXT, "Xd": _emscripten_glGetQueryObjectui64vEXT, "Wd": _emscripten_glGetQueryObjectuivEXT, "Vd": _emscripten_glGetQueryivEXT, "Ud": _emscripten_glGetRenderbufferParameteriv, "Td": _emscripten_glGetShaderInfoLog, "Sd": _emscripten_glGetShaderPrecisionFormat, "Rd": _emscripten_glGetShaderSource, "Qd": _emscripten_glGetShaderiv, "Pd": _emscripten_glGetString, "Od": _emscripten_glGetTexParameterfv, "Nd": _emscripten_glGetTexParameteriv, "Md": _emscripten_glGetUniformLocation, "Ld": _emscripten_glGetUniformfv, "Kd": _emscripten_glGetUniformiv, "Jd": _emscripten_glGetVertexAttribPointerv, "Id": _emscripten_glGetVertexAttribfv, "Hd": _emscripten_glGetVertexAttribiv, "Gd": _emscripten_glHint, "Fd": _emscripten_glIsBuffer, "Ed": _emscripten_glIsEnabled, "Dd": _emscripten_glIsFramebuffer, "Cd": _emscripten_glIsProgram, "Bd": _emscripten_glIsQueryEXT, "Ad": _emscripten_glIsRenderbuffer, "zd": _emscripten_glIsShader, "yd": _emscripten_glIsTexture, "xd": _emscripten_glIsVertexArrayOES, "wd": _emscripten_glLineWidth, "vd": _emscripten_glLinkProgram, "ud": _emscripten_glPixelStorei, "td": _emscripten_glPolygonOffset, "sd": _emscripten_glQueryCounterEXT, "rd": _emscripten_glReadPixels, "qd": _emscripten_glReleaseShaderCompiler, "pd": _emscripten_glRenderbufferStorage, "od": _emscripten_glSampleCoverage, "nd": _emscripten_glScissor, "md": _emscripten_glShaderBinary, "ld": _emscripten_glShaderSource, "kd": _emscripten_glStencilFunc, "jd": _emscripten_glStencilFuncSeparate, "id": _emscripten_glStencilMask, "hd": _emscripten_glStencilMaskSeparate, "gd": _emscripten_glStencilOp, "fd": _emscripten_glStencilOpSeparate, "ed": _emscripten_glTexImage2D, "dd": _emscripten_glTexParameterf, "cd": _emscripten_glTexParameterfv, "bd": _emscripten_glTexParameteri, "ad": _emscripten_glTexParameteriv, "$c": _emscripten_glTexSubImage2D, "_c": _emscripten_glUniform1f, "Zc": _emscripten_glUniform1fv, "Yc": _emscripten_glUniform1i, "Xc": _emscripten_glUniform1iv, "Wc": _emscripten_glUniform2f, "Vc": _emscripten_glUniform2fv, "Uc": _emscripten_glUniform2i, "Tc": _emscripten_glUniform2iv, "Sc": _emscripten_glUniform3f, "Rc": _emscripten_glUniform3fv, "Qc": _emscripten_glUniform3i, "Pc": _emscripten_glUniform3iv, "Oc": _emscripten_glUniform4f, "Nc": _emscripten_glUniform4fv, "Mc": _emscripten_glUniform4i, "Lc": _emscripten_glUniform4iv, "Kc": _emscripten_glUniformMatrix2fv, "Jc": _emscripten_glUniformMatrix3fv, "Ic": _emscripten_glUniformMatrix4fv, "Hc": _emscripten_glUseProgram, "Gc": _emscripten_glValidateProgram, "Fc": _emscripten_glVertexAttrib1f, "Ec": _emscripten_glVertexAttrib1fv, "Dc": _emscripten_glVertexAttrib2f, "Cc": _emscripten_glVertexAttrib2fv, "Bc": _emscripten_glVertexAttrib3f, "Ac": _emscripten_glVertexAttrib3fv, "zc": _emscripten_glVertexAttrib4f, "yc": _emscripten_glVertexAttrib4fv, "xc": _emscripten_glVertexAttribDivisorANGLE, "wc": _emscripten_glVertexAttribPointer, "vc": _emscripten_glViewport, "k": _emscripten_longjmp, "uc": _emscripten_memcpy_big, "tc": _emscripten_pause_main_loop, "sc": _emscripten_resize_heap, "Z": _emscripten_set_main_loop_arg, "rc": _emscripten_thread_sleep, "Kf": _environ_get, "Jf": _environ_sizes_get, "Ba": _exit, "C": _fd_close, "Da": _fd_read, "La": _fd_seek, "$": _fd_write, "B": _gai_strerror, "e": _getTempRet0, "u": _getaddrinfo, "qc": _gethostbyaddr, "pc": _gethostbyname, "t": _getnameinfo, "r": _gettimeofday, "Aa": _glActiveTexture, "N": _glAttachShader, "h": _glBindBuffer, "Y": _glBindFramebuffer, "A": _glBindRenderbuffer, "M": _glBindTexture, "oc": _glBlendFunc, "za": _glBufferData, "ya": _glBufferSubData, "xa": _glCheckFramebufferStatus, "nc": _glClear, "mc": _glClearColor, "lc": _glClearDepthf, "kc": _glClearStencil, "jc": _glColorMask, "L": _glCompileShader, "q": _glCompressedTexImage2D, "p": _glCompressedTexSubImage2D, "wa": _glCreateProgram, "X": _glCreateShader, "ic": _glCullFace, "va": _glDeleteBuffers, "hc": _glDeleteFramebuffers, "K": _glDeleteProgram, "W": _glDeleteRenderbuffers, "z": _glDeleteShader, "ua": _glDeleteTextures, "gc": _glDepthFunc, "fc": _glDepthMask, "ec": _glDisable, "dc": _glDisableVertexAttribArray, "cc": _glDrawArrays, "bc": _glDrawElements, "ac": _glEnable, "ta": _glEnableVertexAttribArray, "$b": _glFlush, "V": _glFramebufferRenderbuffer, "_b": _glFramebufferTexture2D, "sa": _glGenBuffers, "Zb": _glGenFramebuffers, "J": _glGenRenderbuffers, "Yb": _glGenTextures, "Xb": _glGetActiveUniform, "Wb": _glGetAttribLocation, "c": _glGetError, "U": _glGetIntegerv, "ra": _glGetProgramInfoLog, "y": _glGetProgramiv, "qa": _glGetShaderInfoLog, "I": _glGetShaderiv, "x": _glGetString, "Vb": _glGetUniformLocation, "T": _glLinkProgram, "pa": _glPixelStorei, "Ub": _glPolygonOffset, "Tb": _glReadPixels, "S": _glRenderbufferStorage, "Sb": _glScissor, "H": _glShaderSource, "Rb": _glStencilFunc, "Qb": _glStencilMask, "Pb": _glStencilOp, "o": _glTexImage2D, "G": _glTexParameteri, "n": _glTexSubImage2D, "Ob": _glUniform1i, "Nb": _glUniform4fv, "Mb": _glUniformMatrix4fv, "oa": _glUseProgram, "na": _glVertexAttribPointer, "Lb": _glViewport, "Kb": _glfwAccelerometerEnable, "Jb": _glfwCloseWindow, "Ib": _glfwGetAcceleration, "ma": _glfwGetDefaultFramebuffer, "Hb": _glfwGetJoystickButtons, "Gb": _glfwGetJoystickDeviceId, "Fb": _glfwGetJoystickHats, "F": _glfwGetJoystickParam, "Eb": _glfwGetJoystickPos, "Db": _glfwGetKey, "l": _glfwGetMouseButton, "Cb": _glfwGetMousePos, "Bb": _glfwGetMouseWheel, "R": _glfwGetProcAddress, "Ab": _glfwGetWindowParam, "zb": _glfwGetWindowRefreshRate, "la": _glfwGetWindowSize, "yb": _glfwIconifyWindow, "xb": _glfwInitJS, "wb": _glfwOpenWindow, "Q": _glfwOpenWindowHint, "vb": _glfwPollEvents, "ub": _glfwResetKeyboard, "tb": _glfwSetCharCallback, "sb": _glfwSetGamepadCallback, "rb": _glfwSetMarkedTextCallback, "qb": _glfwSetTouchCallback, "pb": _glfwSetWindowCloseCallback, "ob": _glfwSetWindowFocusCallback, "nb": _glfwSetWindowIconifyCallback, "ka": _glfwSetWindowSize, "mb": _glfwSetWindowSizeCallback, "ja": _glfwShowKeyboard, "lb": _glfwSwapBuffers, "ia": _glfwSwapInterval, "kb": _glfwTerminate, "jb": _gmtime_r, "w": invoke_ii, "E": invoke_iii, "P": invoke_iiii, "Ka": invoke_jii, "v": invoke_vi, "D": invoke_vii, "ha": invoke_viii, "g": invoke_viiii, "s": invoke_viiiii, "ib": _js_addBusinessEvent, "hb": _js_addDesignEvent, "gb": _js_addDesignEventWithValue, "fb": _js_addErrorEvent, "eb": _js_addProgressionEvent, "db": _js_addProgressionEventWithScore, "cb": _js_addResourceEvent, "bb": _js_configureAvailableCustomDimensions01, "ab": _js_configureAvailableCustomDimensions02, "$a": _js_configureAvailableCustomDimensions03, "_a": _js_configureAvailableResourceCurrencies, "Za": _js_configureAvailableResourceItemTypes, "Ya": _js_configureBuild, "Xa": _js_configureGameEngineVersion, "Wa": _js_configureSdkGameEngineVersion, "Va": _js_configureUserId, "Ua": _js_endSession, "Ta": _js_getRemoteConfigsContentAsString, "Sa": _js_getRemoteConfigsValueAsStringWithDefaultValue, "Ra": _js_initialize, "Qa": _js_isRemoteConfigsReady, "Pa": _js_setCustomDimension01, "Oa": _js_setCustomDimension02, "Na": _js_setCustomDimension03, "ga": _js_setEnabledInfoLog, "fa": _js_setEnabledVerboseLog, "ea": _js_setEventSubmission, "da": _js_setManualSessionHandling, "Ma": _js_startSession, "a": wasmMemory, "ca": _mktime, "Ja": _pthread_create, "Ia": _pthread_join, "d": _setTempRet0, "Ha": _signal, "Ga": _strftime, "Fa": _sysconf, "Ea": _system, "m": _time };
var asm = createWasm();
var ___wasm_call_ctors = Module["___wasm_call_ctors"] = function() { return (___wasm_call_ctors = Module["___wasm_call_ctors"] = Module["asm"]["_f"]).apply(null, arguments) };
var _dmExportedSymbols = Module["_dmExportedSymbols"] = function() { return (_dmExportedSymbols = Module["_dmExportedSymbols"] = Module["asm"]["$f"]).apply(null, arguments) };
var _main = Module["_main"] = function() { return (_main = Module["_main"] = Module["asm"]["ag"]).apply(null, arguments) };
var _free = Module["_free"] = function() { return (_free = Module["_free"] = Module["asm"]["bg"]).apply(null, arguments) };
var _malloc = Module["_malloc"] = function() { return (_malloc = Module["_malloc"] = Module["asm"]["cg"]).apply(null, arguments) };
var _defos_emit_event_from_js = Module["_defos_emit_event_from_js"] = function() { return (_defos_emit_event_from_js = Module["_defos_emit_event_from_js"] = Module["asm"]["dg"]).apply(null, arguments) };
var _js_remote_configs_callback = Module["_js_remote_configs_callback"] = function() { return (_js_remote_configs_callback = Module["_js_remote_configs_callback"] = Module["asm"]["eg"]).apply(null, arguments) };
var _htonl = Module["_htonl"] = function() { return (_htonl = Module["_htonl"] = Module["asm"]["fg"]).apply(null, arguments) };
var _dmScript_Html5ReportOperationSuccess = Module["_dmScript_Html5ReportOperationSuccess"] = function() { return (_dmScript_Html5ReportOperationSuccess = Module["_dmScript_Html5ReportOperationSuccess"] = Module["asm"]["gg"]).apply(null, arguments) };
var ___errno_location = Module["___errno_location"] = function() { return (___errno_location = Module["___errno_location"] = Module["asm"]["hg"]).apply(null, arguments) };
var _htons = Module["_htons"] = function() { return (_htons = Module["_htons"] = Module["asm"]["ig"]).apply(null, arguments) };
var _ntohs = Module["_ntohs"] = function() { return (_ntohs = Module["_ntohs"] = Module["asm"]["jg"]).apply(null, arguments) };
var _JSWriteDump = Module["_JSWriteDump"] = function() { return (_JSWriteDump = Module["_JSWriteDump"] = Module["asm"]["kg"]).apply(null, arguments) };
var __get_tzname = Module["__get_tzname"] = function() { return (__get_tzname = Module["__get_tzname"] = Module["asm"]["lg"]).apply(null, arguments) };
var __get_daylight = Module["__get_daylight"] = function() { return (__get_daylight = Module["__get_daylight"] = Module["asm"]["mg"]).apply(null, arguments) };
var __get_timezone = Module["__get_timezone"] = function() { return (__get_timezone = Module["__get_timezone"] = Module["asm"]["ng"]).apply(null, arguments) };
var stackSave = Module["stackSave"] = function() { return (stackSave = Module["stackSave"] = Module["asm"]["og"]).apply(null, arguments) };
var stackRestore = Module["stackRestore"] = function() { return (stackRestore = Module["stackRestore"] = Module["asm"]["pg"]).apply(null, arguments) };
var stackAlloc = Module["stackAlloc"] = function() { return (stackAlloc = Module["stackAlloc"] = Module["asm"]["qg"]).apply(null, arguments) };
var _setThrew = Module["_setThrew"] = function() { return (_setThrew = Module["_setThrew"] = Module["asm"]["rg"]).apply(null, arguments) };
var dynCall_jii = Module["dynCall_jii"] = function() { return (dynCall_jii = Module["dynCall_jii"] = Module["asm"]["sg"]).apply(null, arguments) };

function invoke_vii(index, a1, a2) {
    var sp = stackSave();
    try { wasmTable.get(index)(a1, a2) } catch (e) {
        stackRestore(sp);
...
function invoke_ii(index, a1) {
    var sp = stackSave();
    try { return wasmTable.get(index)(a1) } catch (e) {
        stackRestore(sp);
        if (e !== e + 0 && e !== "longjmp") throw e;
        _setThrew(1, 0)
    }
...
function invoke_viiiii(index, a1, a2, a3, a4, a5) {
    var sp = stackSave();
    try { wasmTable.get(index)(a1, a2, a3, a4, a5) } catch (e) {
        stackRestore(sp);
        if (e !== e + 0 && e !== "longjmp") throw e;
        _setThrew(1, 0)
    }
...
function invoke_viiii(index, a1, a2, a3, a4) {
    var sp = stackSave();
    try { wasmTable.get(index)(a1, a2, a3, a4) } catch (e) {
        stackRestore(sp);
        if (e !== e + 0 && e !== "longjmp") throw e;
        _setThrew(1, 0)
    }
...
function invoke_iiii(index, a1, a2, a3) {
    var sp = stackSave();
    try { return wasmTable.get(index)(a1, a2, a3) } catch (e) {
        stackRestore(sp);
        if (e !== e + 0 && e !== "longjmp") throw e;
        _setThrew(1, 0)
    }
...
function invoke_iii(index, a1, a2) {
    var sp = stackSave();
    try { return wasmTable.get(index)(a1, a2) } catch (e) {
        stackRestore(sp);
        if (e !== e + 0 && e !== "longjmp") throw e;
        _setThrew(1, 0)
    }
...
function invoke_viii(index, a1, a2, a3) {
    var sp = stackSave();
    try { wasmTable.get(index)(a1, a2, a3) } catch (e) {
        stackRestore(sp);
        if (e !== e + 0 && e !== "longjmp") throw e;
        _setThrew(1, 0)
    }
...
function invoke_jii(index, a1, a2) {
    var sp = stackSave();
    try { return dynCall_jii(index, a1, a2) } catch (e) {
        stackRestore(sp);
        if (e !== e + 0 && e !== "longjmp") throw e;
        _setThrew(1, 0)
    }
